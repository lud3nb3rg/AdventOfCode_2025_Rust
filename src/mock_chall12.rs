            ...
            for y in initial_y..max_y {
                if can_place(grid, var, x, y) {
                    ...
                    let success = if gift_numbers[index] == 0 {
                        ...
                        // For same shape, continue search from current variation & position to avoid redundant permutations
...
pub fn part1() {
    let content = fs::read_to_string("mock_day12.txt").expect("Should have been able to read the file");
